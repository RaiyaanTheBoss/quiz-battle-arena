...
export class Quiz implements OnDestroy {
  ...
  // Prevent the same result from being saved multiple times
  ...
  this.apiService.saveResult({
    ...
    next: () => console.log('Quiz result saved to MongoDB'),
    error: (error) => console.error('Failed to save quiz result', error)
  });
  ...
  saveResult() {
    ...
    this.apiService.saveResult(result).subscribe({
